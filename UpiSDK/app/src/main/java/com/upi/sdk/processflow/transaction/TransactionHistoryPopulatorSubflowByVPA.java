 ...
 */
public class TransactionHistoryPopulatorSubflowByVPA implements ProcessSubflow {
    ...
    public void execute(Object o, final ProcessChain chain) {
        ...
        if(historyQuery.getFromDate()!=null && historyQuery.getFromDate().length()>0 &&
            ...
            ListParm param = new ListParm();
            param.setParmType(UserEntityType.TRAN_HIST_BY_VPA);
            param.setVpa(historyQuery.getCustomerRefId());
            UppServices.getAllTransactionsByVPA(param, new ServiceCallback<TransactionHistory[]>(TransactionHistory[].class) {
                @Override
                public void onSuccess(TransactionHistory[] result) {
                    // TODO We can convert the bean if required
                    chain.doNext(result);
