...
public class DeleteBeneficiaryConverterSubflow implements ProcessSubflow {

    @Override
    public void execute(Object o, final ProcessChain chain) {

        InputDeleteBeneficiary input = (InputDeleteBeneficiary)o;

        BeneficiaryDetails beneficiaryDetails = null;

        try {
            beneficiaryDetails = Converters.getConverter(InputDeleteBeneficiary.class, BeneficiaryDetails.class).convert(input);
